...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Login status check & Navbar toggle ---
    ...
    if (token) {
        if (loginBtn) {
            ...
            profileBtn.className = 'nav-action-btn profile-btn';
            profileBtn.setAttribute('aria-label', 'Profile');
            profileBtn.setAttribute('title', 'Profile');
            ...
            `;
            loginBtn.parentNode.replaceChild(profileBtn, loginBtn);
        }
    } else {
        if (loginBtn) {
    ...
    // --- Restrict Book Recommended Destination ---
    const bookDestBtns = document.querySelectorAll('.btn-book-dest');
    bookDestBtns.forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            if (!token) {
                ...
                const destName = card.querySelector('.dest-name').textContent.trim();
                const searchParams = {
                    from: "Delhi (DEL)",
                    to: destName,
    ...
    // --- 9. Flight Search Submission Handling ---
    ...
    if (searchForm) {
        searchForm.addEventListener('submit', (e) => {
            ...
            const depDate = document.getElementById('input-dep').value;
            const retDate = document.getElementById('input-ret').value;
            
            const adults = parseInt(document.getElementById('count-adults').textContent) || 1;
            const children = parseInt(document.getElementById('count-children').textContent) || 0;
